...
class BaseSegEdgeDataset(BaseDataset):
    """Segmentation and edge dataset.

    This class supports loading multi-label/binary edge maps along with
    semantic segmentaiton maps.

    Users should modify `edge_mode` args to specify if the edge maps are
    either binary or multi-label or both.
    ...
    Args:
        ...
        bin_edge_suffix (str): Suffix of binary edge maps. Default: '.png'
        mlbl_edge_suffix (str): Suffix of multi-label edge maps. Default: '.png'
        edge_mode (str): Specify the edge mode, either 'binary', 'mlbl' or
            'both'. Default: 'binary'.
    ...
    def __init__(
        ...
        assert edge_mode in [
            ...
            "mlbl",
            "both",
        ], "edge_mode should be either 'binary', 'mlbl' or 'both'."
    ...
    def load_data_list(self) -> List[dict]:
        ...
        bin_edge_dir = self.data_prefix.get("bin_edge_path", None)
        mlbl_edge_dir = self.data_prefix.get("mlbl_edge_path", None)
        ...
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            for line in lines:
                ...
                if seg_dir is not None:
                    ...
                    data_info["seg_map_path"] = osp.join(seg_dir, seg_map)
                if bin_edge_dir is not None and (
                    self.edge_mode == "binary" or self.edge_mode == "both"
                ):
                    bin_edge = img_name + self.bin_edge_suffix
                    data_info["bin_edge_path"] = osp.join(bin_edge_dir, bin_edge)
                if mlbl_edge_dir is not None and (
                    self.edge_mode == "mlbl" or self.edge_mode == "both"
                ):
                    mlbl_edge = img_name + self.mlbl_edge_suffix
                    data_info["mlbl_edge_path"] = osp.join(mlbl_edge_dir, mlbl_edge)
